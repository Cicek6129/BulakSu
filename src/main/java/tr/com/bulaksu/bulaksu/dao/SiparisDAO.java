...
public class SiparisDAO {
    ...
    /**
     * Şube, tarih aralığı, sipariş durumu ve stok durumuna göre JPQL
     * filtreleme. Tüm filtreler opsiyoneldir - null gönderilirse o filtre
     * uygulanmaz.
     *
     ...
     * @param stokDurum Stok durumu: "KRITIK" (kritik seviye altı), "STOKTA"
     * (stokta var), "TUKENMIS" (stok 0) (null = tümü)
     */
    ...
    /**
     * Eski metod - geriye uyumluluk için korundu. Yeni filtreleme metodu
     * findBySubeVeTarihVeDurum kullanılmalı.
     */
    ...
    /**
     * Native SQL ile şube, tarih aralığı ve sipariş tipi filtreleme. Hocanın
     * isteği üzerine Native SQL sorguları kullanılmıştır.
     *
     * @param subeId Şube ID (null = tüm şubeler)
     * @param baslangic Başlangıç tarihi (null = filtresiz)
     * @param bitis Bitiş tarihi (null = filtresiz)
    ...
    public List<Siparis> findByFiltrelerNativeSQL(Integer subeId, LocalDateTime baslangic,
            LocalDateTime bitis, String siparisTipi, Integer urunId) {
        EntityManager em = EntityManagerProvider.getEntityManager();
        try {
            ...
            if ((siparisTipi != null && !siparisTipi.isEmpty()) || urunId != null) {
            ...
            // Ürün filtresi
            if (urunId != null) {
                sql.append("AND sd.urun_Id = :urunId ");
            }
            // Tarih aralığı filtresi
            if (baslangic != null && bitis != null) {
            ...
            if (urunId != null) {
                nativeQuery.setParameter("urunId", urunId);
            }
            if (baslangic != null && bitis != null) {
...
// Sipariş tipi varsa filtreye ekle
            if (siparisTipi != null && !siparisTipi.isEmpty()) {
                jpql.append("AND d.siparisTipi = :siparisTipi ");
            }

// Ürün ID varsa filtreye ekle
            if (urunId != null) {
                // Entity yapınıza göre d.urun.urunId veya d.urun.id yazabilirsiniz
                jpql.append("AND d.urun.urunId = :urunId ");
            }

            jpql.append("ORDER BY s.siparisTarihi DESC");

// Query nesnesini oluştur
            TypedQuery<Siparis> jpqlQuery = em.createQuery(jpql.toString(), Siparis.class)
                    .setParameter("ids", siparisIds);

// Parametreleri dinamik olarak ata
            if (siparisTipi != null && !siparisTipi.isEmpty()) {
                jpqlQuery.setParameter("siparisTipi", siparisTipi);
            }

            if (urunId != null) {
                jpqlQuery.setParameter("urunId", urunId);
            }
